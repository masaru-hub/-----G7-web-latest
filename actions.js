...
function generateTaskId() {
    return 'tk_' + Date.now().toString(36) + '_' + Math.random().toString(36).substring(2, 9);
}
...
window.smartAddTask = async function(worker, name, start, end, target = '', remark = '') {
    ...
    // 1. 基本時刻の計算
    let sISO = toISO(selectedDate, start);
    let eD = selectedDate;
    const sDec = timeToDec(start), eDec = timeToDec(end);
    if (eDec < sDec && end && end !== start) {
        eD = getNextDate(selectedDate);
    }
    let eISO = toISO(eD, end || start);
    
    let newS = new Date(sISO);
    let newE = new Date(eISO);
    
    // 🌟 対象ワーカーの既存タスクを時系列順に取得
    const workerExistingTasks = existing
        .filter(tk => tk.worker === worker)
        .sort((a, b) => (a.startISO || '').localeCompare(b.startISO || ''));
        
    // 🌟 過去への修正・割り込み挿入判定：
    // 実績モードで、終了時刻が未指定または開始時刻と同じ（0分）として送信された場合
    if (currentMode === 'actual' && start === end) {
        // 新規タスクの開始時刻より「後」に開始する既存タスクがあるかチェック
        const laterTasks = workerExistingTasks.filter(tk => new Date(tk.startISO) > newS);
        if (laterTasks.length > 0) {
            // 後続のタスクが存在する場合、この新規タスクは「過去の隙間タスク」であり、
            // 現在進行中タスクではないため、終了時刻を直後のタスクの開始時刻に自動補完
            const nextTk = laterTasks[0];
            end = nextTk.start;
            eISO = nextTk.startISO;
            newE = new Date(eISO);
        }
    }
    ...
    const newItem = {
        ...
        startISO: sISO,
        endISO: eISO,
        duration: getDiffHrs(sISO, eISO),
        target,
        remark
    };
    
    const otherWorkersTasks = existing.filter(tk => tk.worker !== worker);
    const adjustedWorkerTasks = [];
    let newItemInserted = false;
    
    for (const tk of workerExistingTasks) {
        // IDが未設定なら一意なIDを付与
        if (!tk.id) tk.id = generateTaskId();
        
        const tkS = new Date(tk.startISO);
        const tkE = new Date(tk.endISO);
        const isTkLive = (tk.start === tk.end); // 進行中タスク（終了未定）
        
        // 判定A: 進行中タスク（start === end）の保護
        if (isTkLive) {
            // 新規タスクが進行中タスクより前の時間帯なら、進行中タスクは絶対に壊さない
            if (newE <= tkS) {
                adjustedWorkerTasks.push(tk);
                continue;
            }
            // 新規タスクの終了が進行中タスクの開始を侵食している場合、進行中タスクの開始を新規タスクの終了まで押し出す
            if (newE > tkS && newS <= tkS) {
                adjustedWorkerTasks.push({
                    ...tk,
                    start: end,
                    startISO: eISO,
                    duration: "0.0"
        ...
        // 判定B: 時間帯が重複しない場合
        if (tkE <= newS || tkS >= newE) {
            adjustedWorkerTasks.push(tk);
            continue;
        }
        
        // 判定C: 新規タスクが既存タスクに完全に覆われる（既存タスクの中にすっぽり入る） -> 前後に安全分割！
        if (newS > tkS && newE < tkE) {
            ...
            adjustedWorkerTasks.push({
                ...
                endISO: sISO,
                duration: getDiffHrs(tk.startISO, sISO)
            });
            // 新規タスクを挿入
            adjustedWorkerTasks.push(newItem);
            newItemInserted = true;
            // 後半（新しいユニークIDを発行して重複衝突を防止！）
            adjustedWorkerTasks.push({
                ...tk,
                id: generateTaskId(),
                start: end,
                startISO: eISO,
                duration: getDiffHrs(eISO, tk.endISO)
            });
            continue;
        }
        
        // 判定D: 既存タスクが新規タスクに完全に覆われる（新規タスクが既存タスクを丸ごと包む）
        if (newS <= tkS && newE >= tkE) {
            // 既存タスクは新規タスクによって置き換えられるためスキップ
            continue;
        }
        
        // 判定E: 新規タスクが既存タスクの後ろ側（後半）と重なる -> 既存タスクの終了を短縮（休憩等を守る）
        if (newS > tkS && newS < tkE && newE >= tkE) {
            adjustedWorkerTasks.push({
                ...tk,
                end: start,
                endISO: sISO,
        ...
        // 判定F: 新規タスクが既存タスクの前側（前半）と重なる -> 既存タスクの開始を短縮
        if (newS <= tkS && newE > tkS && newE < tkE) {
            adjustedWorkerTasks.push({
                ...tk,
                start: end,
                startISO: eISO,
    ...
    // もし既存タスクの分割でまだ newItem が追加されていない場合、追加する
    if (!newItemInserted) {
        // 直前のタスクが未完了（進行中）だった場合、その終了時刻を新規タスクの開始で締める
        const prevTasks = adjustedWorkerTasks.filter(tk => new Date(tk.startISO) <= newS);
        if (prevTasks.length > 0) {
            const lastPrev = prevTasks[prevTasks.length - 1];
            if (lastPrev.start === lastPrev.end || new Date(lastPrev.endISO) > newS) {
                lastPrev.end = start;
                lastPrev.endISO = sISO;
                lastPrev.duration = getDiffHrs(lastPrev.startISO, sISO);
            }
        }
        adjustedWorkerTasks.push(newItem);
    }
    ...
    // 時間順にソート
    adjustedWorkerTasks.sort((a, b) => (a.startISO || '').localeCompare(b.startISO || ''));
    
    // 全体リストを再構成
    const combinedList = [...otherWorkersTasks, ...adjustedWorkerTasks].sort((a, b) => (a.startISO || '').localeCompare(b.startISO || ''));
    day[targetKey] = combinedList;
    
    // 🌟 【進化】Firestoreへの原子的一括保存（ID重複・Delete/Set衝突ゼロ保証版）
    if (window.kizunaRepo) {
        try {
            await window.kizunaRepo.runBatch(async (batch, sdk) => {
                ...
                // 既存の担当者ドキュメントID集合
                const existingWorkerDocIds = new Set(
                    existing.filter(tk => tk.worker === worker && tk.id).map(tk => tk.id)
                );
                
                // 新しいリストのドキュメントID集合（一意性を担保）
                const newIds = new Set();
                for (const tk of adjustedWorkerTasks) {
                    if (!tk.id || newIds.has(tk.id)) {
                        tk.id = generateTaskId();
                    }
                    newIds.add(tk.id);
                }
                
                // 1. 存在しなくなったドキュメントのみを削除（同じIDに対するdeleteとsetの衝突を完全防止）
                for (const oldId of existingWorkerDocIds) {
                    if (!newIds.has(oldId)) {
                        const oldRef = doc(window.firebaseDB, "daily_logs", selectedDate, targetKey, oldId);
                        batch.delete(oldRef);
                    }
                }
                
                // 2. 新しいリストのドキュメントを書き込み（各IDは必ず1回だけsetされる）
                for (const tk of adjustedWorkerTasks) {
                    const newRef = doc(window.firebaseDB, "daily_logs", selectedDate, targetKey, tk.id);
                    batch.set(newRef, { ...tk });
                }
            });
...
window.syncWorkerTasksToCloud = async function(worker, targetKey) {
    const day = getDayData(selectedDate);
    const workerTasks = (day[targetKey] || []).filter(tk => tk.worker === worker);
    
    // 全タスクにユニークIDを確実に付与
    const seenIds = new Set();
    for (const tk of workerTasks) {
        if (!tk.id || seenIds.has(tk.id)) {
            tk.id = generateTaskId();
        }
        seenIds.add(tk.id);
    }
    
    // 🌟 Firestoreクラウド同期（runBatch）
    if (window.kizunaRepo && window.firebaseDB) {
        try {
            await window.kizunaRepo.runBatch(async (batch, sdk) => {
                const { doc } = sdk;
                const cachedDaily = (data.daily && data.daily[selectedDate] && data.daily[selectedDate][targetKey]) || [];
                const oldWorkerDocIds = new Set(
                    cachedDaily.filter(tk => tk.worker === worker && tk.id).map(tk => tk.id)
                );
                
                // 1. 存在しなくなったドキュメントをFirestoreから削除
                for (const oldId of oldWorkerDocIds) {
                ...
                // 2. 残ったドキュメント（更新・延長後）を書き込み
                for (const tk of workerTasks) {
                    const taskRef = doc(window.firebaseDB, "daily_logs", selectedDate, targetKey, tk.id);
                    batch.set(taskRef, { ...tk });
                }
            });
        } catch (e) {
            console.error("syncWorkerTasksToCloud Error:", e);
        }
    }
    
    // キャッシュを更新
    if (!data.daily) data.daily = {};
    if (!data.daily[selectedDate]) data.daily[selectedDate] = { tasks: [], plans: [] };
    data.daily[selectedDate][targetKey] = [...day[targetKey]];
    
    localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
    if (typeof renderUI === 'function') renderUI();
};

window.editTaskItem = function(t, i, taskId) {
    const d = getDayData(selectedDate);
    const targetKey = t.includes('予') ? 'plans' : 'tasks';
    const list = d[targetKey] || [];
    let item = null;
    if (taskId && taskId !== 'null' && taskId !== 'undefined') {
        item = list.find(x => String(x.id) === String(taskId));
    }
    if (!item && typeof i === 'number' && i >= 0 && i < list.length) {
        item = list[i];
    }
    if (!item) return;
    ...
    // 1. モードを合わせる
    if (window.setMode) {
        window.setMode(t.includes('予') ? 'plan' : 'actual');
    }

    // 2. 担当者をセット
    const wSelect = document.getElementById('worker-select');
    const wIdInput = document.getElementById('worker-id-input');
    const dispWorker = document.getElementById('display-worker');
    if (wSelect) wSelect.value = item.worker;
    if (wIdInput) wIdInput.value = item.worker.replace(/ \[.*?\]$/, '');
    if (dispWorker) dispWorker.innerText = item.worker.replace(/ \[.*?\]$/, '') + " 👤";

    // 3. 業務内容をセット
    const tInput = document.getElementById('task-input');
    const dispTask = document.getElementById('display-task');
    if (tInput) tInput.value = item.name;
    if (dispTask) dispTask.innerText = item.name + " 📝";

    // 4. 相手先・備考をセット
    const targetEl = document.getElementById('task-target');
    const remarkEl = document.getElementById('task-remark');
    if (targetEl) targetEl.value = item.target || '';
    if (remarkEl) remarkEl.value = item.remark || '';

    // 5. 時刻修正エリアを表示し、開始・終了時刻をセット
    const timeArea = document.getElementById('time-settings-area');
    if (timeArea) timeArea.style.display = 'block';

    const sInput = document.getElementById('start-time');
    const eInput = document.getElementById('end-time');
    if (sInput) sInput.value = item.start;
    if (eInput) eInput.value = (item.end && item.end !== item.start) ? item.end : '';

    // 6. 入力カードを表示してスムーズスクロール
    ...
    showToast(`「${item.name}」の時間を編集できます。修正後「お仕事開始」または「登録」を押してください。`, 'info');
};

let pendingDeleteContext = null;

window.deleteItem = async function(t, i, taskId) {
    const d = getDayData(selectedDate); 
    const targetKey = t.includes('予') ? 'plans' : 'tasks';
    const list = d[targetKey] || [];
    
    // 1. 対象タスクを特定（taskId優先、次にインデックス）
    let itemIndex = -1;
    if (taskId && taskId !== 'null' && taskId !== 'undefined') {
        itemIndex = list.findIndex(x => String(x.id) === String(taskId));
    }
    if (itemIndex === -1 && typeof i === 'number' && i >= 0 && i < list.length) {
        itemIndex = i;
    }
    const item = list[itemIndex];
    if (!item) {
        return showToast("削除対象の記録が見つかりませんでした。", "warning");
    }
    
    const worker = item.worker;
    const workerName = worker.replace(/ \[.*?\]$/, '');
    
    // 2. この担当者の同種（実/予）タスクを時間順に取得
    const workerTasks = list.filter(x => x.worker === worker).sort((a, b) => (a.startISO || '').localeCompare(b.startISO || ''));
    const wIdx = workerTasks.findIndex(x => x === item || (item.id && x.id === item.id));
    
    const prevTask = wIdx > 0 ? workerTasks[wIdx - 1] : null;
    const nextTask = (wIdx >= 0 && wIdx < workerTasks.length - 1) ? workerTasks[wIdx + 1] : null;
    
    // 前のタスクと時間が接続しているか？（直前の終了時刻と削除タスクの開始時刻が一致）
    const isPrevConnected = prevTask && (prevTask.end === item.start || prevTask.endISO === item.startISO);
    // 次のタスクと時間が接続しているか？
    const isNextConnected = nextTask && (nextTask.start === item.end || nextTask.startISO === item.endISO);
    
    // パターンA: 前後の作業名が同一（例: 牛加工 → 休憩 → 牛加工）
    const isSameTaskBeforeAndAfter = isPrevConnected && isNextConnected && (prevTask.name === nextTask.name);
    
    // パターンB: 前の作業が存在し、接続している（例: 牛加工 → 休憩）
    const canExtendPrev = isPrevConnected && prevTask;
    ...
    pendingDeleteContext = {
        ...
        prevTask,
        nextTask,
        isSameTaskBeforeAndAfter,
        canExtendPrev,
        worker,
        workerName
    };
    
    openTaskDeleteModal(pendingDeleteContext);
};

window.openTaskDeleteModal = function(ctx) {
    const modal = document.getElementById('task-delete-modal');
    if (!modal) {
        standardDeleteFallback(ctx);
        return;
    }
    
    const infoEl = document.getElementById('task-delete-info');
    const mergeOption = document.getElementById('task-delete-merge-option');
    const mergeTitle = document.getElementById('delete-merge-title');
    const mergeDesc = document.getElementById('delete-merge-desc');
    
    if (infoEl) {
        infoEl.innerHTML = `
            <div style="font-weight: 800; font-size: 14px; color: #1e293b; margin-bottom: 6px;">
                👤 担当: ${ctx.workerName} さん
            </div>
            <div style="display: flex; align-items: center; gap: 8px; font-weight: bold; margin-bottom: 4px;">
                <span class="badge" style="background:#fee2e2; color:#ef4444; font-size:11px;">削除対象</span>
                <span style="color:#e11d48; font-size:14px;">${ctx.item.name}</span>
                <span style="color:#64748b; font-size:13px;">(${ctx.item.start} 〜 ${ctx.item.end})</span>
            </div>
            ${ctx.item.remark ? `<div style="font-size: 12px; color: #64748b; margin-top: 4px;">備考: ${ctx.item.remark}</div>` : ''}
        `;
    }
    
    if (ctx.isSameTaskBeforeAndAfter) {
        // 前後が同じ作業（例: 牛加工 → 休憩 → 牛加工）
        mergeOption.style.display = 'block';
        mergeTitle.innerHTML = `前後の「${ctx.prevTask.name}」を1つに統合する (${ctx.prevTask.start} 〜 ${ctx.nextTask.end})`;
        mergeDesc.textContent = `休憩等の隙間を自動で埋め、ガントチャートを1つの連続した作業に統合します。`;
    } else if (ctx.canExtendPrev) {
        // 直前の作業が存在（例: 牛加工 → 休憩）
        mergeOption.style.display = 'block';
        mergeTitle.innerHTML = `前の「${ctx.prevTask.name}」を ${ctx.item.end} まで自動延長する`;
        mergeDesc.textContent = `ガントチャートの隙間を埋めて、前の作業を ${ctx.prevTask.start}〜${ctx.item.end} に延長します。`;
    } else {
...
window.closeTaskDeleteModal = function() {
    const modal = document.getElementById('task-delete-modal');
    if (modal) modal.style.display = 'none';
    pendingDeleteContext = null;
};

window.executeTaskDelete = async function(mode) {
    ...
    let list = d[targetKey] || [];
    
    if (mode === 'merge') {
        if (ctx.isSameTaskBeforeAndAfter && ctx.prevTask && ctx.nextTask) {
            // 前後が同じタスクの統合（例: 牛加工 09:00-10:00, 休憩 10:00-11:00, 牛加工 11:00-14:00）
            ctx.prevTask.end = ctx.nextTask.end;
            ctx.prevTask.endISO = ctx.nextTask.endISO;
            ctx.prevTask.duration = getDiffHrs(ctx.prevTask.startISO, ctx.prevTask.endISO);
            
            // 削除対象itemとnextTaskをリストから除外
            d[targetKey] = list.filter(x => x !== ctx.item && x !== ctx.nextTask && (!ctx.item.id || x.id !== ctx.item.id) && (!ctx.nextTask.id || x.id !== ctx.nextTask.id));
            
            await syncWorkerTasksToCloud(ctx.worker, targetKey);
            showToast(`「${ctx.item.name}」を削除し、前後の「${ctx.prevTask.name}」を自動で1つに統合しました（${ctx.prevTask.start}〜${ctx.nextTask.end}）。`, 'success');
            return;
        } else if (ctx.canExtendPrev && ctx.prevTask) {
            // 直前のタスクの延長（例: 牛加工 09:00-10:00, 休憩 10:00-11:00）
            ctx.prevTask.end = ctx.item.end;
            ctx.prevTask.endISO = ctx.item.endISO;
            ctx.prevTask.duration = getDiffHrs(ctx.prevTask.startISO, ctx.prevTask.endISO);
            
            // 削除対象itemをリストから除外
            d[targetKey] = list.filter(x => x !== ctx.item && (!ctx.item.id || x.id !== ctx.item.id));
            
            await syncWorkerTasksToCloud(ctx.worker, targetKey);
            showToast(`「${ctx.item.name}」を削除し、前の作業「${ctx.prevTask.name}」を ${ctx.item.end} まで自動延長しました。`, 'success');
            return;
        }
    }
    ...
    d[targetKey] = list.filter(x => x !== ctx.item && (!ctx.item.id || x.id !== ctx.item.id));
    await syncWorkerTasksToCloud(ctx.worker, targetKey);
    showToast(`「${ctx.item.name}」を削除しました。`, 'info');
};

window.standardDeleteFallback = async function(ctx) {
    if (ctx.isSameTaskBeforeAndAfter) {
        const doMerge = confirm(`「${ctx.item.name}」を削除します。\n前後の「${ctx.prevTask.name}」を自動で1つに統合して隙間を埋めますか？\n\n【OK】: 前後の作業を1つに統合する（推奨）\n【キャンセル】: 単に削除する（空き時間にする）`);
        executeTaskDelete(doMerge ? 'merge' : 'simple');
    } else if (ctx.canExtendPrev) {
        const doExtend = confirm(`「${ctx.item.name}」を削除します。\n前の作業「${ctx.prevTask.name}」を ${ctx.item.end} まで自動延長して隙間を埋めますか？\n\n【OK】: 前の作業を自動延長する\n【キャンセル】: 単に削除する（空き時間にする）`);
        executeTaskDelete(doExtend ? 'merge' : 'simple');
    } else {
        if (confirm(`この記録（${ctx.item.name}）を削除してもよろしいですか？`)) {
            executeTaskDelete('simple');
        }
    }
};
...
window.renderModalLists = function() {
    ...
    const annCancelBtn = document.getElementById('ann-cancel-edit-btn');
    if (annCancelBtn) annCancelBtn.style.display = editingAnnId ? 'inline-block' : 'none';
    const questCancelBtn = document.getElementById('quest-cancel-edit-btn');
    if (questCancelBtn) questCancelBtn.style.display = editingQuestId ? 'inline-block' : 'none';

    if (mAnn) {
        mAnn.innerHTML = '<div style="font-size:12px; font-weight:bold; color:#b45309; margin-bottom:10px;">現在の告知一覧</div>' + 
            ((data.announcements && data.announcements.length) ? data.announcements.map(a => `
                <div class="master-item" style="background:#fffbeb; margin-bottom:6px; border:1px solid #fde68a; ${String(editingAnnId) === String(a.id) ? 'outline: 2px solid #f59e0b; box-shadow: 0 0 10px rgba(245, 158, 11, 0.3);' : ''}">
                    <div style="flex:1; font-size:11px; line-height:1.5; white-space: pre-wrap; word-break: break-word;">${a.content}</div>
                    <div style="display:flex; gap:8px;">
                        <button type="button" onclick="event.stopPropagation(); editAnnouncement('${a.id}')" style="background:none; border:none; color:#f59e0b; cursor:pointer; padding:4px;" title="編集"><i data-lucide="edit-2" style="width:14px; pointer-events:none;"></i></button>
                        <button type="button" onclick="event.stopPropagation(); deleteAnnouncement('${a.id}')" style="background:none; border:none; color:#ef4444; cursor:pointer; font-weight:bold; padding:4px; font-size:16px;" title="削除">×</button>
                    </div>
                </div>
    ...
    if (mQue) {
        mQue.innerHTML = '<div style="font-size:12px; font-weight:bold; color:#4338ca; margin-bottom:10px;">現在の依頼一覧</div>' + 
            ((data.quests && data.quests.length) ? data.quests.map(q => `
                <div class="master-item" style="background:#f5f3ff; margin-bottom:6px; border:1px solid #ddd6fe; ${String(editingQuestId) === String(q.id) ? 'outline: 2px solid #6366f1; box-shadow: 0 0 10px rgba(99, 102, 241, 0.3);' : ''}">
                    <div style="flex:1; font-size:11px;"><b>${q.title}</b> <small>(${q.worker ? q.worker.replace(/ \[.*?\]$/, '') : '全体'})</small></div>
                    <div style="display:flex; gap:8px;">
                        <button type="button" onclick="event.stopPropagation(); editQuest('${q.id}')" style="background:none; border:none; color:#6366f1; cursor:pointer; padding:4px;" title="編集"><i data-lucide="edit-2" style="width:14px; pointer-events:none;"></i></button>
                        <button type="button" onclick="event.stopPropagation(); deleteQuest('${q.id}')" style="background:none; border:none; color:#ef4444; cursor:pointer; font-weight:bold; padding:4px; font-size:16px;" title="削除">×</button>
                    </div>
                </div>
    ...
    if (mLnk) {
        mLnk.innerHTML = '<div style="font-size:12px; font-weight:bold; color:#047857; margin-bottom:10px;">現在の連携一覧</div>' + 
            ((data.links && data.links.length) ? data.links.map((l, idx) => `
                <div class="master-item" style="background:#f0fdf4; margin-bottom:6px; border:1px solid #bbf7d0;">
                    ...
                    <div style="display:flex; gap:5px; align-items:center;">
                        ...
                        <span onclick="event.stopPropagation(); deleteLink('${l.id}')" style="color:#ef4444; cursor:pointer; font-weight:bold; padding:0 5px; margin-left:5px;" title="削除">×</span>
                    </div>
                </div>
...
window.moveLink = function(id, dir) {
    const idx = data.links.findIndex(l => String(l.id) === String(id));
    if (idx === -1) return;
    ...
    saveAndRefresh('master'); renderModalLists();
};

window.editAnnouncement = function(id) {
    const ann = (data.announcements || []).find(a => String(a.id) === String(id));
    if (!ann) return;
    editingAnnId = id;
    ...
    if (btn) btn.innerHTML = 'キセキのお知らせを更新 ✏️';
    const cancelBtn = document.getElementById('ann-cancel-edit-btn');
    if (cancelBtn) cancelBtn.style.display = 'inline-block';
    const modalContent = document.querySelector('#announcement-modal .modal-content');
...
window.cancelEditAnnouncement = function() {
    editingAnnId = null;
    ['ann-content', 'ann-expiry', 'ann-link', 'ann-file'].forEach(id => { const el = document.getElementById(id); if(el) el.value = ''; });
    const prev = document.getElementById('selected-ann-files-preview');
    if (prev) prev.innerHTML = '';
    const btn = document.querySelector('#announcement-modal button[onclick*="saveAnnouncement"]');
    if (btn) btn.innerHTML = '告知を飛ばす';
    const cancelBtn = document.getElementById('ann-cancel-edit-btn');
...
window.editQuest = function(id) {
    const q = (data.quests || []).find(qu => String(qu.id) === String(id));
    if (!q) return;
    editingQuestId = id;
    const modal = document.getElementById('quest-modal');
    if (modal) modal.style.display = 'flex';
    const sel = document.getElementById('quest-worker-select');
    if (sel && (!sel.options || sel.options.length === 0)) {
        sel.innerHTML = '<option value="全体">👥 全員へ</option>' + data.masterWorkers.map(w => `<option value="${w}">${w.replace(/ \[.*?\]$/, '')}</option>`).join('');
    }
    ...
    if (workerEl) workerEl.value = q.worker || '全体';
    ...
    if (btn) btn.innerHTML = '依頼を更新する ✏️';
    const cancelBtn = document.getElementById('quest-cancel-edit-btn');
    if (cancelBtn) cancelBtn.style.display = 'inline-block';
    const modalContent = document.querySelector('#quest-modal .modal-content');
    if (modalContent) modalContent.scrollTop = 0;
    renderModalLists();
};

window.cancelEditQuest = function() {
    editingQuestId = null;
    ['quest-title', 'quest-content', 'quest-file', 'quest-link', 'quest-expiry'].forEach(id => { const el = document.getElementById(id); if(el) el.value = ''; });
    const prev = document.getElementById('selected-files-preview');
    if (prev) prev.innerHTML = '';
    const imp = document.getElementById('quest-important');
    if (imp) imp.checked = false;
    const btn = document.querySelector('#quest-modal button[onclick*="saveQuest"]');
    if (btn) btn.innerHTML = 'クエストを発令';
    const cancelBtn = document.getElementById('quest-cancel-edit-btn');
    if (cancelBtn) cancelBtn.style.display = 'none';
    renderModalLists();
};
...
window.saveAnnouncement = function() {
    ...
    if (editingAnnId) { 
        const ann = (data.announcements || []).find(a => String(a.id) === String(editingAnnId)); 
        if (ann) { 
            ann.content = content; 
            ann.expiresAt = expiry || ''; 
            ann.link = link || ''; 
            if (fileInfo) ann.file = fileInfo; 
            showToast("キセキのお知らせを更新しました。", 'success'); 
        } else {
            showToast("お知らせが見つかりませんでした。", 'error');
        }
    } else { 
        ...
        showToast("キセキのお知らせを送信しました。", 'success'); 
    }
...
window.saveQuest = function() {
    ...
    if (editingQuestId) { 
        const q = (data.quests || []).find(qu => String(qu.id) === String(editingQuestId)); 
        if (q) { 
            q.title = title; 
            q.content = content || ''; 
            q.worker = worker || '全体'; 
            ...
            if (fileInput && fileInput.files && fileInput.files.length > 0) { 
                const files = []; 
                for (let f of fileInput.files) files.push({ name: f.name, size: (f.size / 1024).toFixed(1) + 'KB' }); 
                q.files = files; 
            } 
            showToast("キセキの依頼を更新しました。", 'success'); 
        } else {
            showToast("依頼が見つかりませんでした。", 'error');
        }
    } else { 
        const files = []; 
        if (fileInput && fileInput.files && fileInput.files.length > 0) { 
        ...
        showToast("キセキの依頼を発令しました。", 'success'); 
    }
...
window.markAsRead = function(id, workerName) { 
    const ann = (data.announcements || []).find(a => String(a.id) === String(id)); 
    if (ann && !ann.readBy.includes(workerName)) { 
        ann.readBy.push(workerName); 
        if (ann.readBy.length >= data.masterWorkers.length && !ann.expiresAt) { 
            data.announcements = data.announcements.filter(a => String(a.id) !== String(id)); 
            window.activeAnnId = null; 
            celebrate(); 
            showToast("全員が確認したため、告知を終了しました。", 'success'); 
        } else { 
            showToast(`${workerName.replace(/ \[.*?\]$/, '')}さんの確認を記録しました。`, 'success'); 
        } 
        saveAndRefresh('master'); 
    } 
};

window.closeKizunaConfirmModal = function() {
    const modal = document.getElementById('kizuna-confirm-modal');
    if (modal) modal.style.display = 'none';
};

window.kizunaConfirm = function({ title, message, icon = '✨', okText = 'OK', cancelText = 'キャンセル', okColor = '#6366f1', onConfirm }) {
    const modal = document.getElementById('kizuna-confirm-modal');
    if (!modal) {
        if (typeof onConfirm === 'function') onConfirm();
        return;
    }
    const iconEl = document.getElementById('kizuna-confirm-icon');
    const titleEl = document.getElementById('kizuna-confirm-title');
    const msgEl = document.getElementById('kizuna-confirm-message');
    const okBtn = document.getElementById('kizuna-confirm-ok-btn');
    const cancelBtn = document.getElementById('kizuna-confirm-cancel-btn');
...
window.deleteQuest = (id) => { 
    ...
    const title = quest ? `「${quest.title}」` : 'この依頼';

    kizunaConfirm({
        icon: '🎉',
        title: 'クエスト完了確認',
        message: `${title} の任務は完了しましたか？\n完了すると達成として記録・終了されます✨`,
        okText: '任務完了！✨',
        okColor: 'linear-gradient(135deg, #10b981 0%, #059669 100%)',
        onConfirm: async () => {
            data.quests = (data.quests || []).filter(q => String(q.id) !== String(id)); 
            if (String(editingQuestId) === String(id)) {
                if (typeof cancelEditQuest === 'function') cancelEditQuest();
            }
            if (typeof celebrate === 'function') celebrate();
            showToast(`${title} を完了しました！お疲れ様でした👏✨`, 'success'); 
            await saveAndRefresh('master'); 
            if (typeof renderModalLists === 'function') renderModalLists();
        }
    });
};
...
window.deleteAnnouncement = (id) => {
    const ann = (data.announcements || []).find(a => String(a.id) === String(id));
    const preview = ann ? (ann.content.length > 25 ? ann.content.substring(0, 25) + '...' : ann.content) : 'この告知';
    kizunaConfirm({
        icon: '📢',
        title: '告知の終了確認',
        message: `「${preview}」を終了しますか？\n終了するとお知らせ一覧から取り下げられます。`,
        okText: '告知を終了する',
        okColor: '#f59e0b',
        onConfirm: async () => {
            data.announcements = (data.announcements || []).filter(a => String(a.id) !== String(id));
            if (String(editingAnnId) === String(id)) {
                if (typeof cancelEditAnnouncement === 'function') cancelEditAnnouncement();
            }
            showToast("告知を終了しました。", 'success');
            await saveAndRefresh('master');
            if (typeof renderModalLists === 'function') renderModalLists();
        }
    });
...
window.deleteLink = (id) => {
    const link = (data.links || []).find(l => String(l.id) === String(id));
    const title = link ? `「${link.title}」` : 'このリンク';
    kizunaConfirm({
        icon: '🔗',
        title: '連携リンクの削除確認',
        ...
        okColor: '#ef4444',
        onConfirm: async () => {
            data.links = (data.links || []).filter(l => String(l.id) !== String(id));
            showToast("リンクを削除しました。", 'info');
            await saveAndRefresh('master');
            if (typeof renderModalLists === 'function') renderModalLists();
        }
    });
...
window.renderKizuna = function() {
    ...
    const isAnnActive = (a) => !a.expiresAt || window.calculateDaysRemaining(a.expiresAt) >= 0;
    const annCount = data.announcements.filter(isAnnActive).length, questCount = data.quests.length, linkCount = data.links.length;
    ...
    if (currentKizunaTab === 'ann') {
        ...
        // 📢 周知優先ソート！
        const allAnn = data.announcements.filter(isAnnActive);
        ...
        annArea.innerHTML = sortedAnn.map(a => {
            ...
            let expiryBadge = ""; if (a.expiresAt) { const diff = window.calculateDaysRemaining(a.expiresAt); const color = diff <= 0 ? "#ef4444" : diff <= 3 ? "#f59e0b" : "#10b981"; expiryBadge = `<span style="background:${color}; color:white; padding:2px 8px; border-radius:20px; font-size:10px; font-weight:900;">${diff===0?"本日締切":diff<0?"期限終了":`残り ${diff} 日`}</span>`; }
            return `<div class="announcement-card ${isNew?'new-arrival-card':''}" style="border-top: 4px solid #f59e0b;">${a.expiresAt?'<div class="ann-badge">期限付き</div>':''}<div style="display:flex; justify-content:space-between; align-items:flex-start;"><div><div style="font-weight: 800; color: #b45309; display:flex; align-items:center; gap:6px;">キセキのお知らせ ${isNew?'<span class="floating-star-inline"><svg class="new-star-icon-inline" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg></span>':''}</div>${expiryBadge}</div><div style="display:flex; align-items:center; gap:6px;"><button onclick="editAnnouncement('${a.id}')" class="btn-small btn-outline" style="padding:2px 8px; font-size:11px; font-weight:bold; color:#b45309; border-color:#fde68a; background:white;" title="お知らせを変更"><i data-lucide="edit-2" style="width:12px; height:12px; vertical-align:middle; margin-right:2px;"></i>編集</button><div style="font-size:10px; color:#94a3b8;">${new Date(a.id).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</div></div></div><div class="announcement-content" style="font-size: 14px; margin: 10px 0; color:#475569; white-space: pre-wrap; word-break: break-word; line-height: 1.6;">${a.content}</div><div style="display:flex; gap:6px; flex-wrap:wrap; margin-bottom:12px;">${a.link?`<a href="${a.link}" target="_blank" class="btn-small" style="background:#fff7ed; color:#b45309; border:1px solid #fde68a;"><i data-lucide="external-link" style="width:12px;"></i> リンク</a>`:''}${a.file?`<div class="btn-small" style="background:#fef3c7; color:#d97706;"><i data-lucide="file-text" style="width:12px;"></i> ${a.file.name}</div>`:''}</div><div style="background:#fffbeb; border-radius:10px; padding:10px; border:1px solid #fef3c7;"><div style="display:flex; justify-content:space-between; align-items:center;"><button onclick="window.activeAnnId = (window.activeAnnId === ${a.id} ? null : ${a.id}); renderUI();" class="btn-small" style="background:#f59e0b; width:auto;">確認 (${unread.length})</button><div style="font-size:10px; color:#d97706; font-weight:bold;">未受領 ${unread.length}名</div></div>${isShowList?`<div style="margin-top:10px; display:flex; flex-wrap:wrap; gap:5px;">${unread.map(w=>`<button onclick="markAsRead(${a.id},'${w}')" class="btn-small" style="background:white; color:#475569; border:1px solid #fde68a;">${w.replace(/ \[.*?\]$/, '')}</button>`).join('')}</div>`:''}</div></div>`;
        }).join('') || 'お知らせはありません';
    }
    
    if (currentKizunaTab === 'quest') {
        qArea.style.display = 'grid';
        // ⚔️ 各クエストにIDが存在することを保証
        (data.quests || []).forEach((q, idx) => {
            if (!q.id && q.id !== 0) q.id = String(Date.now() + idx);
        });
        // ⚔️ 指揮官ソート発動！
        const sorted = [...data.quests].sort((a, b) => {
            ...
            // 第二位：期限付き任務を期限が近い順に
            if (a.expiry && b.expiry) return window.calculateDaysRemaining(a.expiry) - window.calculateDaysRemaining(b.expiry);
        ...
        qArea.innerHTML = sorted.map(q => {
            const isPersonal = q.worker !== '全体', isImportant = !!q.important, workerName = isPersonal ? (q.worker ? q.worker.replace(/ \[.*?\]$/, '') : '') : '全員', isNew = isToday(q.createdAt);
            let deadlineHtml = ""; if (q.expiry) { const diff = window.calculateDaysRemaining(q.expiry); const color = diff <= 0 ? "#ef4444" : diff <= 3 ? "#f59e0b" : "#10b981"; deadlineHtml = `<div style="position:absolute; top:10px; right:10px; background:${color}; color:white; padding:2px 8px; border-radius:20px; font-size:10px; font-weight:900;">${diff===0?"本日締切":diff<0?"期限終了":`残り ${diff} 日`}</div>`; }
